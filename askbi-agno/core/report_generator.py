...
📅 更新日期: 2026.04.09
📝 变更说明: 完全对齐rule_ori/generate_report.py的验证规则
...
def extract_city(location):
    ...
    province_city_patterns = [
        (r'山东省济南', '济南'), (r'山东省青岛', '青岛'), (r'山东省烟台', '烟台'),
        (r'山东省威海', '威海'), (r'山东省潍坊', '潍坊'), (r'山东省淄博', '淄博'),
        (r'山东省德州', '德州'), (r'山东省临沂', '临沂'), (r'山东省泰安', '泰安'),
        (r'山东省济宁', '济宁'), (r'山东省菏泽', '菏泽'), (r'山东省滨州', '滨州'),
        (r'山东省聊城', '聊城'), (r'山东省枣庄', '枣庄'), (r'山东省日照', '日照'),
        (r'山东省东营', '东营'),
        (r'广东省广州', '广州'), (r'广东省深圳', '深圳'), (r'广东省东莞', '东莞'),
        (r'广东省佛山', '佛山'), (r'广东省珠海', '珠海'),
        (r'浙江省杭州', '杭州'), (r'浙江省宁波', '宁波'), (r'浙江省温州', '温州'),
        (r'江苏省南京', '南京'), (r'江苏省无锡', '无锡'), (r'江苏省苏州', '苏州'),
        (r'江苏省常州', '常州'),
        (r'北京市', '北京'), (r'上海市', '上海'), (r'天津市', '天津'), (r'重庆市', '重庆'),
        (r'四川省成都', '成都'), (r'湖北省武汉', '武汉'), (r'湖南省长沙', '长沙'),
        (r'陕西省西安', '西安'), (r'安徽省合肥', '合肥'), (r'云南省昆明', '昆明'),
        (r'广西省南宁', '南宁'), (r'广西南宁', '南宁'),
        (r'内蒙古呼和浩特', '呼和浩特'), (r'黑龙江省哈尔滨', '哈尔滨'), (r'吉林省长春', '长春'),
    ]
...
def generate_hr_attendance_report(
    detail_file_path: str,
    summary_file_path: str,
    output_file_path: str,
    project_trip_mapping: Optional[List[Dict]] = None
) -> Dict[str, Any]:
    """
    生成人事考勤报表
    ...
    try:
        # ========== 读取数据 ==========
        if detail_file_path == summary_file_path:
            df_detail = pd.read_excel(detail_file_path, sheet_name='考勤明细表')
            df_summary = pd.read_excel(summary_file_path, sheet_name='考勤汇总表')
        else:
        ...
        for _, emp_row in emp_info.iterrows():
            ...
            ps_gx = (pr_ys + cc_days_work * 2 - cc_deduct) / month_count
            ...
            zm_gx = (zl_ys + zr_ys + cc_days_rest * 2 - cc_deduct_rest) / month_count
            emp['周末贡献'] = round(zm_gx, 2) if zm_gx != 0 else 0

            # === 第24列: 法定节假日带薪贡献 ===
            jr_ys = sum_rows['假日延时合计'].sum()
            jr_gx = jr_ys / month_count
        ...
        summary_lines = [
            f"✓ 报表生成成功，共{len(result_df)}人",
            f"  - 输出文件: {os.path.basename(output_file_path)}",
            f"  - 数据行数: {len(result_df)}",
            f"  - 数据列数: {len(columns_order)}",
        ]

        if problem_count > 0:
            summary_lines.append(f"  ⚠ 发现 {problem_count} 个问题人员，请重点关注")

        summary_text = "\n".join(summary_lines)

        return {
            "success": True,
            "message": f"报表生成成功，共{len(result_df)}人",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        result = generate_hr_attendance_report(DATA_FILE, DATA_FILE, OUTPUT_FILE)
    elif len(sys.argv) == 2:
        input_file = sys.argv[1]
        output_file = '报表结果.xlsx'
        result = generate_hr_attendance_report(input_file, input_file, output_file)
    elif len(sys.argv) >= 4:
        detail_file = sys.argv[1]
        summary_file = sys.argv[2]
        output_file = sys.argv[3]
        result = generate_hr_attendance_report(detail_file, summary_file, output_file)
    else:
        print("用法:")
        print("  python generate_report.py <数据文件>")
        print("  python generate_report.py <明细表> <汇总表> <输出文件>")
        sys.exit(1)
